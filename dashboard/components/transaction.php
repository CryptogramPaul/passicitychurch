 <?php
    ...
    $sql = $conn->prepare("SELECT intentions_id, booking_id, sacrament_type, booking_date, start_time , booking_status, amount_to_pay, wedding_id, burial_id, baptism_id, total_payment, (amount_to_pay - total_payment) as balance
        FROM booking 
        WHERE customer_id = ? ORDER BY posted ASC
       ");
    $sql->execute([$_COOKIE['customer_id']]);
    $count = $sql->rowCount();
?>
 <div class="row">
     <div class="col-12">
             ...
             <table class="table table-bordered table-hover text-center">
                 ...
                 <tbody>
                    ...
                    ?>
                             ...
                             <div class="d-flex ">
                                 <?php 
                                    ...
                                    if ($row['sacrament_type'] == 'Baptism') {
                                        ...
                                        $target = 'show_wedding';
                                        $id = $row['wedding_id'];
                                    }else if($row['sacrament_type'] == 'Intentions'){
                                        $target = 'show_intentions';
                                        $id = $row['intentions_id'];
                                    }
                                ?>
                                 <?php if($row['booking_status'] == 'Pending'){ ?>
                                 <!-- data-bs-toggle="modal" data-bs-target="#<?php echo $target ?>" -->
                                 <a class="badge bg-info text-white text-decoration-none badge-info"
                                     onclick="EditTransaction(`<?php echo $id ?>`,`<?php echo $row['booking_id'] ?>`,`<?php echo $target ?>`)"
                                     title="Edit">
                                     <i class="fa fa-pencil p-1"></i>
                                 </a>
                                 &nbsp;&nbsp;&nbsp;&nbsp;
                                 <a class="badge bg-danger text-white text-decoration-none badge-danger "
                                     onclick="DeleteTransaction(`<?php echo $id ?>`,`<?php echo $row['sacrament_type'] ?>`)"
                                     title="Delete">
                     ...
                     <?php
                        if($count == 0){
                            echo '<tr><td colspan="9" class="text-center">No transaction found.</td></tr>';
                        }
                     ?>
                 </tbody>
             </table>
